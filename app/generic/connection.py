...
class SafeRequest():
    ...
    def get_request_from(self, url):
        ...
        Returns:
            requests response object containing request data
        """
        _s = requests.Session()
        logging.debug('Establishing Connection')
        try:
            _resp = self.requests_retry_session(session=_s).get(url, headers=self.headers, verify=self.verify)
            _resp.raise_for_status()
        except Exception as _x:
            logging.exception('Connection failed: %s', (_x))
        else:
            logging.debug('Connection Successful: %s', (_resp.status_code))
            return _resp

    def get_json_from(self, url):
        _resp = self.get_request_from(url)
        return _resp.json()
    ...
    def get_image_from(self, url):
        _resp = self.get_request_from(url)
        return send_file(BytesIO(_resp.content), attachment_filename="image.jpeg")

    def get_headers_from(self, url):
        _resp = self.get_request_from(url)
